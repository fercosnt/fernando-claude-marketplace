---
name: deck-reviewer
description: Revisa STORYBOARD de deck com 3 criticos adversariais (clareza/persuasao/SUCCESs Heath) e gera issues BLOCKER/MAJOR/MINOR. Use ao pedir "/deck review", "revisa esse deck", "vamos criticar storyboard".
intent: action
effort: medium
nb_ids:
...
- Criticos aplicados: clareza + persuasao + SUCCESs
...
{Uma frase: "Pronto pra ensaio" | "Resolver N blocker(s) antes de apresentar" | "Re-rodar deck-{vertical} v2 com correcoes"}
```
...
## Workflow (3 passes sequenciais)

```
1. PARSE        Le STORYBOARD + extrai Meta (vertical, max_ctas, slides)
...
5. CONSOLIDA    Agrega issues, classifica severidade, ordena por slide
6. ESCREVE      Renderiza review-skeleton.md no path paralelo
```

Cada critico roda em sequencia (nao paralelo — eles compartilham contexto e o segundo precisa ver issues do primeiro pra nao duplicar).
...
Usa `assets/templates/review-skeleton.md`, substitui placeholders, grava em `{path-paralelo}.review.md`.
...
4 cases canonicos em [evals/reviewer-cases.md](evals/reviewer-cases.md):
...
DoD §13.6 exige 3+ passando. Alvo: 4/4.

## DoD especifico

- Frontmatter §10.7 com `intent: action`, `effort: medium`, NB1 listado
- 4 eval cases passando
- 3 criticos como passes sequenciais (clareza → persuasao → SUCCESs)
...
- [evals/reviewer-cases.md](evals/reviewer-cases.md) — 4 eval cases
